...
// ---- player level / XP: earnings grant XP, level-ups boost the earn multiplier ----
const lvlBefore = h.debug().lvl, multBefore = h.debug().lvlMult;
h.addXP(100000);   // overflow well past the cap to exercise the level-up loop
assert(h.debug().lvl > lvlBefore, "earning XP raises the player level");
assert(h.debug().lvlMult > multBefore, "leveling up boosts the earnings multiplier");
assert(h.state.ach.includes("lvl10") && h.state.ach.includes("lvl25"), "level milestone achievements unlock");
assert(h.debug().lvl >= 25, "high XP pushes well past level 1");
