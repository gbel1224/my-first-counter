...
- **Ocean life:** three sharks patrol the deep water — swim out too far and one hunts you (warning toast, fin wake, bites for real damage; escape to the shallows or a vehicle). Ambient beachgoers bob and paddle in the shallows and scatter shoreward when a shark prowls near. A **jet-ski race circuit** ("Wake Breaker") slaloms the coastline with a floating checkered start line by the rental skis — same checkpoint/medal system as the street races.
- **Fishing:** stop any boat or jet ski on the water and a CAST button appears — cast a bobbing line, wait for the tug ("❗ Something's biting"), and reel within the window to land a weighted catch table from Mackerel ($18) up to the rare Golden Koi ($350). Reel too early and you lose it.
- **Treasure dives:** six sunken treasures hide in the deep water (shark territory), each marked by a golden shimmer on the surface and a gold dot on the minimap. Swim onto one and the action button becomes DIVE — you plunge underwater in a stream of bubbles and surface with $250–800. Collected spots persist in the save.
...
- **Wanted level (1–5 stars):** crimes raise heat; police cars chase and PIT-ram you. At 4 stars a police helicopter joins; at 5 stars an army tank. Die or get busted and you pay a fine.
- **Police investigation:** the force hunts a *belief* about where you are, not your coordinates. Units run a real line-of-sight test against the city's buildings; the belief only refreshes while somebody actually has eyes on you. Break sight and it rots where it stands — they converge on your **last known position**, then sweep outward in a ring that widens the longer their information goes stale. Heat cools only while you're genuinely unseen, so ducking an alley, cutting a corner or stepping inside a building is a real escape rather than a countdown. Nobody shoots through walls; the helicopter still spots you from above. The stars go hollow with a 🔍 while they're searching.
...
- **Heists (the set-piece score):** call one in from the phone and pick an approach — **Loud** (bigger take, 4 stars the instant the alarm trips, fast vault) or **Quiet** (smaller take, only 2 stars, slower vault). The run has four stages: **case** the target (a real named landmark — the club, the marina, the depot…), **park a getaway car** on a staging corner (it refuses you on foot), **crack the vault** while an alarm screams and the bag fills — walk out early and you keep only what you bagged — then **run it to the drop**. The payout is loot × approach, minus the crew's cut, **plus a 45% clean-getaway bonus for arriving with the police having genuinely lost your trail**. Getting wasted or busted mid-run loses the score.
...
- **The phone:** a real handset, styled like an iPhone — titanium bezel, dynamic island, squircle app icons, a home-indicator bar you tap to go back, status bar with the in-world clock and a LIVE marker when you're wanted, a home screen of apps, and an unread badge on the button so the city can get your attention.
  - **💼 Jobs** — the on-demand job board (below).
  - **🌴 Palmgram** — a city social feed you can **like and post to**, that reacts to *what you actually did*. Anything that came from something you did also raises a **notification banner** over the HUD, so the city reaching out finds you rather than waiting to be checked. The heat rising, the police losing your trail, a heist going off, a rampage banking, a level-up: each posts from a named Palm City account with a timestamp and a like count ("PCPD sweeping midtown. suspect gone. absolutely gone."). Between those it fills with ordinary city chatter — beach cams, traffic, gossip — so it reads as a place rather than a notification log.
  - **🏦 Bank** — deposit and withdraw, and a balance that earns interest (0.6%/min, compounding). The reason it matters: **death and bust fines only ever take the cash in your pocket**, so banking your fortune before a heist is a real decision rather than a chore. **Term deposits** lock a sum for three minutes at +8%, with an early-break option that returns the principal and forfeits the interest. A **statement** lists every movement newest-first with the balance it left behind, which is what makes the interest visible rather than a number quietly drifting upward.
  - **📈 Stocks** — six Palm City tickers, each tied to a business you can actually walk into (PALM, BUNS, NEON, WASH, BAY, and VIGL private security). Prices drift on a mean-reverting random walk you can buy and sell into. The hook: **robbing a business craters its stock (−22%)**, so the score you just pulled is also a tip-off you could have traded on, and a big rampage lifts the security ticker.
  - **📇 Contacts** — people who do something when you call: **Marco** tells you what you should be doing, **Hire Muscle** puts an armed ally on your side, and the **Mechanic** has your own car brought to you for $500.
...
- **Dynamic street events (freeplay):** the world spontaneously spawns a random event near you every ~45-90s. Seven flavors: a **Cash Drop** (grab it), a **Rush Delivery** (timed pickup→dropoff), a **Bounty** (catch a fleeing target on foot or by car), an **Armored Heist in progress** (run down a fleeing armored truck and crack it for big loot), a **Gang Shootout** (reach the firefight — gangsters trade muzzle fire — to muscle in on the score), a **Police Chase** (intercept a fleeing runner car to help or hijack), and a **Street Race** flashing "JOIN NOW" (reach the start line before it rolls out). Each drops a tall pulsing light beacon you follow via the normal objective marker/minimap/HUD, with a countdown and a cash reward. Keeps freeplay from ever feeling empty.
